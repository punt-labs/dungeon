...
1. Call `recall` to check for existing game state
2. If it returns `"NO_SAVE_FILE"` (or the player said "new"), start a new game
3. If state exists, parse it, call `unfurl_scroll` for the adventure script, and process the player's action
...
- **`new`**: Call `obliterate`. Show title screen and script selection.
...
When starting a new game, call `scry` for the title screen ASCII art, then call `quest_board` to get available adventures.
...
1. **Recall state** via `recall` (parse YAML frontmatter from the returned content)
2. **Unfurl the scroll** via `unfurl_scroll` with the script name from state
...
9. **Inscribe state** via `inscribe` with updated scene, turn+1, updated health/inventory/flags
...
- **ASCII art is optional per scene.** Use it when the script provides it. You may also call `scry` for common situations (entrance, death, treasure, etc.).
